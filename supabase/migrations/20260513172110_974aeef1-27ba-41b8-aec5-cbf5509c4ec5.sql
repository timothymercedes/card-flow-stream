...
-- Bid de-duplication hardening + payment-event idempotency
-- 1) Hard guarantee: a single bidder cannot win the same (stream, round, amount) twice
CREATE UNIQUE INDEX IF NOT EXISTS uq_live_bids_dedupe
  ON public.live_bids (stream_id, bidder_id, round_number, amount);

-- 2) Server-side idempotency window inside place_live_bid:
--    if the same buyer just placed the exact same amount within 2 seconds
--    (double-tap, double network send, retry from offline buffer), treat
--    it as a no-op success instead of throwing or double-counting.
...
BEGIN
  ...
  -- Idempotency: same buyer + same stream + same exact amount within 2s = duplicate request
  SELECT id INTO _dup_id FROM public.live_bids
   WHERE stream_id = _stream_id AND bidder_id = _uid AND amount = _amount
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Hit uq_live_bids_dedupe (same buyer/round/amount). Treat as already-applied.
    RETURN jsonb_build_object('ok', true, 'amount', _amount, 'duplicate', true);
  END;
...
-- 3) Stream payment events idempotency: prevent the same (order_id, event_type)
--    being recorded twice within 30 seconds (e.g. a Stripe webhook retry that
--    slips past event-id dedupe due to a different event id but same effect).
CREATE UNIQUE INDEX IF NOT EXISTS uq_stream_payment_events_idem
  ON public.stream_payment_events (order_id, event_type)
