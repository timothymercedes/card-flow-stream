import { Link } from "@tanstack/react-router";
import { useEffect, useState } from "react";
import { supabase } from "@/integrations/supabase/client";
import { Store as StoreIcon } from "lucide-react";

type Props = {
  sellerId?: string | null;
  username?: string | null;
  avatarUrl?: string | null;
  shopName?: string | null;
  size?: "sm" | "md";
  /** If false, render as plain text (use when nested inside another Link). */
  linkable?: boolean;
  className?: string;
};

const cache = new Map<string, { username: string; avatar_url: string | null; shop_name: string | null }>();

export function SellerBadge({ sellerId, username, avatarUrl, shopName, size = "sm", linkable = true, className = "" }: Props) {
  const [data, setData] = useState<{ username: string; avatar_url: string | null; shop_name: string | null } | null>(
    username ? { username, avatar_url: avatarUrl ?? null, shop_name: shopName ?? null } : null
  );

  useEffect(() => {
    if (data?.username) return;
    if (!sellerId) return;
    if (cache.has(sellerId)) { setData(cache.get(sellerId)!); return; }
    supabase.from("profiles")
      .select("username, avatar_url, shop_name")
      .eq("id", sellerId)
      .maybeSingle()
      .then(({ data: p }) => {
        if (!p) return;
        const v = { username: p.username || "seller", avatar_url: p.avatar_url, shop_name: p.shop_name };
        cache.set(sellerId, v);
        setData(v);
      });
  }, [sellerId, data?.username]);

  if (!data?.username) return null;

  const avatarSize = size === "md" ? "h-7 w-7" : "h-5 w-5";
  const textSize = size === "md" ? "text-sm" : "text-xs";

  const userInner = (
    <>
      {data.avatar_url ? (
        <img src={data.avatar_url} className={`${avatarSize} rounded-full object-cover`} alt={data.username} />
      ) : (
        <div className={`${avatarSize} rounded-full bg-muted`} />
      )}
      <span className={`${textSize} font-semibold text-foreground`}>@{data.username}</span>
    </>
  );

  const shopInner = data.shop_name && (
    <span className={`inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 ${textSize} font-bold text-primary`}>
      <StoreIcon className="h-3 w-3" /> {data.shop_name}
    </span>
  );

  if (!linkable) {
    return (
      <div className={`inline-flex items-center gap-2 ${className}`}>
        <span className="flex items-center gap-1.5">{userInner}</span>
        {shopInner}
      </div>
    );
  }

  return (
    <div className={`inline-flex items-center gap-2 ${className}`}>
      <Link to="/seller/$username" params={{ username: data.username }} className="flex items-center gap-1.5 hover:opacity-80" onClick={(e) => e.stopPropagation()}>
        {userInner}
      </Link>
      {data.shop_name && (
        <Link to="/seller/$username" params={{ username: data.username }} className="hover:opacity-80" onClick={(e) => e.stopPropagation()}>
          {shopInner}
        </Link>
      )}
    </div>
  );
}
