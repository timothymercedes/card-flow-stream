 ...
 *
 * Shows upcoming queued items (with images, starting bids, buy-now), the
 * current top pre-bid leader, and lets signed-in viewers place a pre-bid
 * before the item goes live. Realtime: re-renders on any auction_queue or
 * prebids change for this stream.
 */
...
import { X, Bookmark, Gavel, ListOrdered, Trophy } from "lucide-react";
    ...
    async function refresh() {
      const [q, p] = await Promise.all([
        supabase.from("auction_queue" as any).select("*").eq("stream_id", streamId)
          .in("status", ["queued"]).order("position", { ascending: true }),
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/60 backdrop-blur-sm md:items-center" onClick={onClose}>
      ...
      >
                ...
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[11px] text-muted-foreground">
                      Start ${Number(it.starting_bid).toFixed(0)} · {it.duration_seconds}s
        ...
        <div className="border-t border-border bg-muted/30 px-4 py-2 text-center text-[10px] text-muted-foreground">
          ...
          Pre-bids are saved for the host to see. Items go live in queue order.
        </div>
      </div>
    </div>
  );
