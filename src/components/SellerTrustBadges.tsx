import { BadgeCheck, Zap, Rocket, ShieldCheck, Star } from "lucide-react";
...
export function SellerTrustBadges({ sellerId, compact = false }: { sellerId: string; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const { data } = await (supabase.rpc as any)("get_seller_badges", { _seller_id: sellerId });
